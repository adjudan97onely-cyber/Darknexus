...
export default function LotteryWorkbench({ lottery, title, description, accent }) {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setRecommendations(Array.isArray(recsRes?.data?.data) ? recsRes.data.data : []);
        setGrids(Array.isArray(gridsRes?.data?.data) ? gridsRes.data.data : []);
        setLatest(latestRes?.data || {});
        setHistory(Array.isArray(historyRes?.data?.data) ? historyRes.data.data : []);
      } catch (loadError) {
        console.error('Lottery load error:', loadError);
  ...
  async function handleAutoSelect() {
    try {
      ...
      const selections = Array.isArray(response.data?.data) ? response.data.data : [];
      setAutoSelected(selections);
    } catch (loadError) {
  ...
  return (
    <div className="premium-shell space-y-8">
      ...
      <section className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
        <div className="premium-panel p-6">
          ...
          <div className="mt-5 space-y-3">
            ...
            {(Array.isArray(autoSelected) ? autoSelected : []).map((grid) => (
        ...
        <div className="premium-panel p-6">
          ...
          <div className="mt-5 space-y-3">
            {(Array.isArray(history) ? history : []).slice(0, 8).map((draw) => (
      ...
      <section className="grid gap-6 lg:grid-cols-2">
        <div className="premium-panel p-6">
          ...
          <div className="mt-5 grid gap-3">
            {Array.isArray(recommendations) && recommendations.slice(0, 6).map((item) => (
      ...
      <RecentResultsWidget lotteryType={lottery} />
    </div>
  );
}
