...
export default function SportsAnalyzer() {
  ...
  // Load initial data - Ligues et Stats
  useEffect(() => {
    const loadBase = async () => {
      try {
        ...
        setLeagues(Array.isArray(leagueRes.data?.data) ? leagueRes.data.data : []);
  ...
  // Load filtered data - Matchs et Recommandations
  useEffect(() => {
    const loadByFilter = async () => {
      try {
        ...
        setMatches(Array.isArray(matchRes.data?.data) ? matchRes.data.data : []);
        setRecommendations(Array.isArray(recRes.data?.data) ? recRes.data.data : []);
      } catch (err) {
        console.error('Sports filter error:', err);
        setMatches([]);
        setRecommendations([]);
      }
    };
  ...
  const countries = useMemo(() => [...new Set((Array.isArray(leagues) ? leagues : []).map((item) => item.country))], [leagues]);
  const availableLeagues = useMemo(() => (Array.isArray(leagues) ? leagues : []).filter((item) => !selectedCountry || item.country === selectedCountry), [leagues, selectedCountry]);
      ...
      <section className="premium-panel grid gap-4 p-6 lg:grid-cols-4">
        ...
        <label className="premium-field">
          ...
          <select value={selectedLeague} onChange={(event) => setSelectedLeague(event.target.value)}>
            ...
            {availableLeagues.map((league) => <option key={league.name} value={league.name}>{league.name}</option>)}
          </select>
        </label>
        <label className="premium-field">
      ...
      <section className="grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
        ...
        <div className="premium-panel p-6">
          <div className="mb-5 flex items-center justify-between">
            ...
            <span className="premium-badge">{Array.isArray(recommendations) ? recommendations.length : 0} signaux</span>
          </div>
