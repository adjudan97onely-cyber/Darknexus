...
const WhisperVoiceInput = ({ onTranscript, disabled = false, showTranscript = false }) => {
  ...
  React.useEffect(() => {
    // VÃ©rifier si le navigateur supporte MediaRecorder
  ...
  const startRecording = async () => {
    try {
      ...
      mediaRecorder.onstop = async () => {
        ...
        // Envoyer Ã  Whisper pour transcription
      ...
      toast({
        ...
        description: "Parlez naturellement ! Comme avec Emergent ðŸ”¥"
      });
      ...
      if (error.name === 'NotAllowedError') {
        toast({
          title: "âŒ AccÃ¨s micro refusÃ©",
          description: "Autorisez l'accÃ¨s au microphone dans les paramÃ¨tres",
          variant: "destructive"
        });
      } else {
        toast({
  ...
  const transcribeAudio = async (audioBlob) => {
    try {
      ...
      if (response.data.success) {
        ...
        toast({
          title: "âœ… Transcription rÃ©ussie !",
          description: `${transcribedText.split(' ').length} mots capturÃ©s avec Whisper ðŸ”¥`
        });
      } else {
        throw new Error('Transcription Ã©chouÃ©e');
      }

    } catch (error) {
      console.error('Erreur transcription:', error);
      toast({
        title: "âŒ Erreur de transcription",
        description: error.response?.data?.detail || "RÃ©essayez",
        variant: "destructive"
      });
    } finally {
  ...
  return (
    <div className="flex items-center gap-2">
      <Button
        ...
        title={isRecording ? "ArrÃªter et transcrire" : "Enregistrer avec Whisper"}
      >
        {isTranscribing ? (
